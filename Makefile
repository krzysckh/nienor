...
bin/nienor: nienor.scm nienor/*.scm nienor/lib/*.scm bin
	$(OL) $(OLFLAGS) -x c -o - nienor.scm | $(CC) $(CFLAGS) -o bin/nienor -x c - $(maybe_sqlite) -lm -lpthread
bin/nienor.exe: nienor.scm nienor/*.scm nienor/lib/*.scm bin
	wine $(OL_WIN) -x c -o - nienor.scm | i686-w64-mingw32-gcc $(CFLAGS) -o bin/nienor.exe -I/usr/local/include -x c - -lm -lopengl32 -lwinmm -lgdi32 -lws2_32
bin:
	mkdir -p bin
clean:
	rm -f bin/nienor
