        ...
        (letrec ((walk
                  (λ (stack types code)
                    (cond
                     ...
                     ((imm? code)  (values stack types))
                     ((and (list? code) (eq? (car code) '_push!))
                      ;; (when (eq? (cadr code) 'byte)
                      ;;   (warn "fuck, byte in " code name))
                      (values (cons (caddr code) stack) types))
                     ...
                     ((and (list? code) (eq? (car code) '_with-locals!))
                      (lwalk walk (caddr code) stack types))
                     ...
                     ((list? code)
                      ...
                      (lwalk walk (cdr code) stack types))
                     (else
                      (error "WTF (what that funcall):" code))))))
          (lwalk walk body #n (if argT (list->ff (zip cons args argT)) empty)))))

    (define (typecheck code env)
      (let ((defuns (code->defuns code))
            (walk (C walk-typecheck env)))
        (for-each (λ (x) (lets ((_1 _2 (walk x))) #t)) defuns)))
